...
public sealed class GameEntity
{
   ...
   // private Vector3 pos;
    ...
    public CatchTy catchty;
    public bool isCatchPian { get; private set; }
    ...
    public void CheckFishHook(object data, bool isReach)
    {
       ...
       // Debug.Log("CurrentPosX----" + currentposX + "-----MinX-----" + min + "-------maxX----" + max + "====pos====" + posX);
        if (currentposX >= min && currentposX <= max)
        {
            self.SetActive(false);
            catchty = CatchTy.Catch;
            LuckyBoyMgr.Instance.gameXP = this;
            tempImg.transform.localPosition = self.transform.localPosition;
            tempImg.transform.SetParent(t.parent);
            tempImg.overrideSprite = sp;
            tempImg.gameObject.SetActive(true);
            ...
            catchty = CatchTy.CatchErrorPos;
        isCatchPian = currentposX >= min - catchPian && currentposX <= max + catchPian;
    }
    ...
    public void LeftRightMove()
    {
        ...
        self.transform.DOMoveX(x, 0.07f).SetLoops(5, LoopType.Yoyo);
    }
    /// <summary>
    /// 旋转自己
    /// </summary>
    ...
    {
        if (this != LuckyBoyMgr.Instance.gameXP)
        {
            float distance = Vector3.Distance(self.transform.localPosition, tempImg.gameObject.transform.localPosition);
            if (distance < 95)
            {
