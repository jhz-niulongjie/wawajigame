...
public sealed class UIMgr : Singleton<UIMgr>
{
    ...
    private UIViewBase RegisterDlgScripte(string viewName)
    {
        ...
        if (!string.IsNullOrEmpty(viewName))
        {
            ...
            switch (viewName)
            {
                case UIBigBomPage.NAME: view = uiRoot.AddComponent<UIBigBomPage>(); break;
                case UIBigBomResultPage.NAME: view = uiRoot.AddComponent<UIBigBomResultPage>(); break;
                case UIBigBomCodePage.NAME: view = uiRoot.AddComponent<UIBigBomCodePage>(); break;
            }
    ...
    {
        UIViewBase view = GetView(viewName);
        if (view)
        {
            view.SetData(data);
            if (isShow)
