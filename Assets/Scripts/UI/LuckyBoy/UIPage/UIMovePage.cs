...
public sealed class UIMovePage : UIDataBase
{
    ...
    private bool isStartDrop = false;
    private List<GameEntity> list = new List<GameEntity>();//轮子
    public List<GameEntity> listPang = new List<GameEntity>();//小胖
    ...
    private void UnReg()
    {
        ...
        EventHandler.UnRegisterEvent(EventHandlerType.GameEndStart, GameEndStart);
        EventHandler.UnRegisterEvent(EventHandlerType.XiaoPang_incline, XiaoPang_incline);
    }
        ...
        if (modelWheel && parentWheel)
        {
            ...
            if (modelPang && parentPang)
            {
                ...
                for (int i = 0; i < 4; i++)
                {
                    ...
                    sp = UIAtlasManager.Instance.LoadSprite(UIAtlasName.UIMain, new_name);
                    tempObj.transform.GetComponent<Image>().overrideSprite = sp;
    ...
    void Update()
    {
        ...
        if (isStartDrop)
        {
            for (int i = 0; i < listPang.Count; i++)
                if (listPang[i] != null)
                    listPang[i].RotateSelf();
        }
    }

    private float GetSpeed()
    {
        if (sdk.randomQuXian == 1) return 160 * Time.deltaTime;
        else if (sdk.randomQuXian == 2) return 400 * Time.deltaTime;
        else
        {
    ...
    {
        sdk.gameStatus.SetIsDouDong(false);
        sdk.startCarwTime = CommTool.GetTimeStamp();
        sdk.gameStatus.SetRecordTime(sdk.startCarwTime);
        ...
        foreach (var item in listPang)
        {
            ...
            {
                sdk.gameStatus.SetIsDouDong(true);
    ...
    //游戏结束
    private void GameEndStart(object data)
    {
        Stop = (bool)data;
    }
    private void XiaoPang_incline(object data)
    {
        isStartDrop = (bool)data;
    }
    ...
    private bool ComputeTimes()
    {
        ...
        //bool flag = catchCount >= sdk.winningTimes;
        //if (playrCatch >= sdk.carwBasicCount)
        //{
